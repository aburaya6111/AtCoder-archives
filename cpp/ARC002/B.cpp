...
void Main() {
    ...
    date theDay1 = boost::gregorian::from_string(s);
    REP(i, 1 << 30) {
        string ss = to_iso_extended_string(theDay1 + days(i));

        vector<string> v = split(ss);
        if (atoi(v[0].c_str()) % atoi(v[1].c_str()) == 0 && (atoi(v[0].c_str()) / atoi(v[1].c_str())) % atoi(v[2].c_str()) == 0) {
            cout << to_iso_extended_string(theDay1 + days(i));
            return;
        }
    }
}
